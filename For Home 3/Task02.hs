...
sortN n = helper n 0 9 
 where
  ...
  helper num result currDigit
   ...
   | currDigit == 0 = result
   | otherwise = helper num (concatDigit result currDigit (countDigitsOcc num currDigit)) (currDigit - 1)
...
countDigitsOcc :: Int -> Int -> Int
countDigitsOcc n m = helper n 0
 where
    helper :: Int -> Int -> Int
    helper 0 count
     | m == 0 = count + 1
...
concatDigit :: Int -> Int -> Int -> Int
concatDigit x y z = helper x y z
 where
    helper :: Int -> Int -> Int -> Int
    helper num _ 0 = num
